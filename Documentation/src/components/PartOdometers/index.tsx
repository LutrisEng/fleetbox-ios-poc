...
import { Category, LineItemTypesSchema, Type } from '@site/src/generated/LineItemTypes'
...
function getCategoryTypes(category: Category): Type[] {
    return (category.subcategories ?? []).reduce((acc, cat) => [...acc, ...getCategoryTypes(cat)], category.types ?? [])
}

const types = lineItemTypes.categories.reduce((acc, cat) => [...acc, ...getCategoryTypes(cat)], [])
        ...
        <ul>
            { lineItemTypes.components.map(component => (
                <li className={styles.listitem} key={ component.id }>
                    <p>{ component.name }</p>
                    { component.filter ? <p>Filter: { componentsById[component.filter].name }</p> : null }
                    <p>Line item types which reset this part odometer:</p>
                    <ul>
                        { types.filter(type => type.replaces === component.id).map(type => (
